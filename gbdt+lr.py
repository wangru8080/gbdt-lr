...
def lr_predict(data, category_feature, continous_feature): # 0.47181
    # 连续特征归一化
    print('开始归一化...')
    scaler = MinMaxScaler()
...
def gbdt_predict(data, category_feature, continous_feature): # 0.44548
    # 离散特征one-hot编码
    print('开始one-hot...')
    for col in category_feature:
        onehot_feats = pd.get_dummies(data[col], prefix = col)
    ...
    tr_logloss = log_loss(y_train, gbm.predict(x_train))
    val_logloss = log_loss(y_val, gbm.predict(x_val))
...
def gbdt_lr_predict(data, category_feature, continous_feature): # 0.43616
    # 离散特征one-hot编码
    print('开始one-hot...')
    for col in category_feature:
        onehot_feats = pd.get_dummies(data[col], prefix = col)
...
def gbdt_ffm_predict(data, category_feature, continous_feature):
    pass

if __name__ == '__main__':
    data = preProcess()
    continous_feature = ['I'] * 13
    continous_feature = [col + str(i + 1) for i, col in enumerate(continous_feature)] 
    category_feature = ['C'] * 26
    category_feature = [col + str(i + 1) for i, col in enumerate(category_feature)] 
    # lr_predict(data, category_feature, continous_feature)
    # gbdt_predict(data, category_feature, continous_feature)
    gbdt_lr_predict(data, category_feature, continous_feature)
